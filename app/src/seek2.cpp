...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Título encontrado no índice - ID do artigo: " << result_entry.id
            << std::endl;
  ...
  std::cout << "Total de blocos lidos (índice + dados): "
            ...
            << std::endl;
  std::cout << "Blocos do índice secundário: " << index_stats.total_blocos
            << std::endl;
  std::cout << "Tempo total de busca: " << duration.count() << " µs"
            << std::endl;
  ...
  std::cout << std::endl << "=== ANÁLISE DE BUSCA POR TÍTULO ===" << std::endl;
  std::cout << "Busca por título é indireta:" << std::endl;
  std::cout << "1. Busca no índice secundário (Título -> ID)" << std::endl;
  std::cout << "2. Busca no arquivo de dados (ID -> Registro completo)"
            << std::endl;
  std::cout << "Total de acessos: "
            << (index_stats.blocos_lidos + data_stats.blocos_lidos) << " blocos"
            << std::endl;
