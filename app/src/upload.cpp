...
int main(int argc, char *argv[])
{
    ...
    while (std::getline(csv_file, line))
    {
        ...
        try
        {
            ...
            if (fields.size() < 7)
            {
                std::cerr << "Linha inválida (poucos campos): " << records_processed << std::endl;
                continue;
            }
            ...
            if (hash_table.insert(artigo))
            {
                ...
                IndicePrimarioEntry primary_entry(id, hash_position);
                IndiceSecundarioEntry secondary_entry(titulo, id);
                ...
                {
                    std::cout << "Inseridos: " << records_inserted << " registros..." << std::endl;
                }
            }
            else
            {
    ...
    std::cout << "Taxa de inserção: " << (records_inserted * 1000.0 / duration.count()) << " registros/s" << std::endl;
    ...
    std::cout << "Índice secundário (B+Tree): " << secondary_index_file << std::endl;
